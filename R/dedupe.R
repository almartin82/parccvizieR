...
#'
#' @param pv_srf a parcvizieR summative record file
#'
#' @return SRF with a rn column where 1 = best test record to use
#' @export

dedupe_srf <- function(pv_srf) {
  ...
  #rank.  for all the dummies above, TRUE is bad, FALSE is good.
  pv_srf <- pv_srf %>%
    group_by(state_student_identifier, test_code, academic_year) %>%
    mutate(
      rn = rank(
        order(is_void, is_assigned, is_not_tested, is_not_attempted, desc(total_test_items_attempted))
      )
    ) %>%
    select(
